...
interface ClassTreeEditorProps {
  initialData?: any;
...
interface ClassTreeEditorState {
...
class ClassTreeEditor extends Component<ClassTreeEditorProps, ClassTreeEditorState> {
  constructor(props: ClassTreeEditorProps) {
    super(props);
    this.state = {
      nodes: []
    };
  ...
  componentDidUpdate(prevProps: ClassTreeEditorProps) {
    if (prevProps.initialData !== this.props.initialData || 
        prevProps.fieldDescriptions !== this.props.fieldDescriptions) {
      this.initializeNodes();
    }
  }

  initializeNodes = () => {
    const { initialData = {}, fieldDescriptions = {} } = this.props;
    ...
    this.setState({ nodes: applyDescriptions(initialNodes) }, () => {
      this.notifyChange();
    });
  }
  ...
  notifyChange = () => {
    ...
    if (onChange) {
      const data = nodesToJson(this.state.nodes);
      ...
      collectDescriptions(this.state.nodes);
      onChange(data, descriptions);
    }
  }

  updateNode = (nodeId: string, updates: Partial<SimpleFieldNode>) => {
    ...
    this.setState({ nodes: updateInTree(this.state.nodes) }, () => {
      this.notifyChange();
    });
  }
  ...
  deleteNode = (nodeId: string) => {
    ...
    this.setState({ nodes: removeFromTree(this.state.nodes) }, () => {
      this.notifyChange();
    });
  }
  ...
  addChild = (parentId: string) => {
    ...
    this.setState({ nodes: addToTree(this.state.nodes) }, () => {
      this.notifyChange();
    });
  }

  addRootNode = () => {
    const newKey = `newField${this.state.nodes.length + 1}`;
    const newNode: SimpleFieldNode = {
      id: generateId(),
      key: newKey,
      type: 'string',
  ...
  toggleExpand = (nodeId: string) => {
    ...
    const foundNode = this.state.nodes.find(n => findNodeById(n, nodeId));
    if (foundNode) {
      const targetNode = findNodeById(foundNode, nodeId);
      this.updateNode(nodeId, { expanded: !(targetNode && targetNode.expanded) });
    }
  }

  renderNode = (node: SimpleFieldNode, siblings: SimpleFieldNode[]): React.ReactNode => {
      ...
      <div key={node.id}>
        <div style={{
          ...
          <Input
            size="small"
            value={node.key}
            onChange={(e) => this.updateNode(node.id, { key: e.target.value })}
          ...
          <Select
            size="small"
            value={node.type}
            onChange={(type: SimpleFieldType) => this.updateNode(node.id, { type })}
          ...
          <Button
            ...
            onClick={() => this.updateNode(node.id, { useMock: !node.useMock })}
          >
          ...
          <div style={{ marginLeft: 8, width: 120 }}>
            {config.canHaveChildren ? (
              ...
              <Select
                size="small"
                value={node.mockValue}
                onChange={(mockValue) => this.updateNode(node.id, { mockValue: mockValue as string })}
              ...
              <Select
                ...
                onChange={(value) => this.updateNode(node.id, { value: value === 'true' })}
                style={{ width: '100%' }}
              >
          ...
          <Input
            ...
            onChange={(e) => this.updateNode(node.id, { description: e.target.value })}
            placeholder="描述"
            style={{ width: 100, marginLeft: 8 }}
          />
          ...
          <div style={{ marginLeft: 8, display: 'flex', gap: 4 }}>
            {config.canHaveChildren && (
              <Button size="small" onClick={() => this.addChild(node.id)}>+</Button>
            )}
            <Button size="small" onClick={() => this.deleteNode(node.id)} style={{ color: '#ff4d4f' }}>×</Button>
          </div>
        </div>
        ...
        {node.children && node.expanded && (
          <div>
            {node.children.map(child => this.renderNode(child, node.children!))}
          </div>
        )}
      </div>
    );
    ...
    return (
      <div style={{ 
        ...
        <div style={{ 
          ...
          <Button size="small" onClick={this.addRootNode}>添加字段</Button>
        </div>
        ...
        <div style={{ 
          ...
          {nodes.length === 0 ? (
            ...
            nodes.map(node => this.renderNode(node, nodes))
          )}
        </div>
      </div>
    );
...
export default ClassTreeEditor;
